...
const PropertyPanel: React.FC = () => {
  ...
  // Load selected node or connection data
  useEffect(() => {
    ...
    if (selectedConnectionId && canvas) {
      const selectedConnection = canvas.connections.find(conn => conn.id === selectedConnectionId);
  ...
  }, [selectedNodeId, selectedConnectionId, canvas]);
  ...
  const getConnectedNodes = () => {
    ...
    return canvas.connections
      .filter(conn => conn.sourceId === selectedNodeId || conn.targetId === selectedNodeId)
      .map(conn => {
        const otherNodeId = conn.sourceId === selectedNodeId ? conn.targetId : conn.sourceId;
        const otherNode = canvas.nodes.find(node => node.id === otherNodeId);
  ...
  const renderConnectionProperties = () => {
    ...
    const sourceNode = canvas.nodes.find(node => node.id === selectedConnection.sourceId);
    const targetNode = canvas.nodes.find(node => node.id === selectedConnection.targetId);
    ...
    <div className="w-72 bg-[#1E1E1E] border-l border-[#333] overflow-y-auto shadow-lg" data-panel="properties">
      <div className="p-4 border-b border-[#333]">
        <h2 className="font-medium text-lg">
          {selectedNodeId ? 'Node Properties' : 
      ...
      <div className="p-4">
        {selectedNodeId ? renderNodeProperties() :
         selectedConnectionId ? renderConnectionProperties() :
         renderEmptyState()}
      </div>
    </div>
  );
